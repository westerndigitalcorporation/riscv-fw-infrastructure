...
void rtosalUpdateStackPriorContextSwitch(volatile rtosalStack_t** pxTopOfStack)
{
   ...
   if (pUiNewSP != NULL)
   {
      ...
      pUiNewSP -= (D_RTOSAL_NUM_OF_STACK_ENTRIES);
      /* save on to task stack the values of t3 (comrv stack address), mepc and tp */
      pUiNewSP[D_RTOSAL_T3_INDEX_ON_TASK_STACK]   = (*pxTopOfStack)[D_RTOSAL_T3_INDEX_ON_TASK_STACK];
      pUiNewSP[D_RTOSAL_MEPC_INDEX_ON_TASK_STACK] = (*pxTopOfStack)[D_RTOSAL_MEPC_INDEX_ON_TASK_STACK];
      pUiNewSP[D_RTOSAL_TP_INDEX_ON_TASK_STACK]   = (u32_t)pComrvTaskStackRegsVal;
