...
*                            handler
* @param uiAutoActivate     - D_RTOSAL_AUTO_START timer shall be activated when
...
#elif D_USE_THREADX
   uiRes = tx_timer_create((TX_TIMER*)pRtosalTimerCb->cTimerCB, pRtosTimerName, fptrRtosTimerCallcabk,
                   uiTimeCallbackParam, uiTicks, uiRescheduleTicks, uiAutoActivate);
#endif /* #ifdef D_USE_FREERTOS */
...
#elif D_USE_THREADX
   uiRes = tx_timer_delete((TX_TIMER*)pRtosalTimerCb->cTimerCB);
#endif /* #ifdef D_USE_FREERTOS */
...
#elif D_USE_THREADX
   uiRes = tx_timer_activate((TX_TIMER*)pRtosalTimerCb->cTimerCB);
#endif /* #ifdef D_USE_FREERTOS */
...
#elif D_USE_THREADX
   uiRes = tx_timer_deactivate((TX_TIMER*)pRtosalTimerCb->cTimerCB);
#endif /* #ifdef D_USE_FREERTOS */
...
u32_t rtosTimerModifyPeriod(rtosalTimer_t* pRtosalTimerCb, u32_t uiTicks, u32_t uiRescheduleTicks)
{
...
#elif D_USE_THREADX
   /* get timer info */
   uiRes = tx_timer_info_get((TX_TIMER*)pRtosalTimerCb->cTimerCB, TX_NULL, &uiActive, TX_NULL, TX_NULL, TX_NULL);
   if (uiRes == D_RTOSAL_SUCCESS)
   {
      ...
      if (uiRes == D_RTOSAL_SUCCESS)
      {
         ...
         uiRes = tx_timer_change((TX_TIMER*)pRtosalTimerCb->cTimerCB, uiTicks, uiRescheduleTicks);
